...
public void fillHists(){
	H_elec_mom[e_sect].fill(e_mom);
	H_elec_vz_mom[e_sect].fill(e_mom,e_vz);
	H_elec_theta_mom[e_sect].fill(e_mom,e_theta)
	H_elec_theta_phi[e_sect].fill(e_phi,e_theta);
}
...
public void makeElectron(DataBank recPart){
		...
		e_phi = (float) Math.toDegrees(Math.toDegrees(VE.phi()))
		e_theta = (float) Math.toDegrees(Math.toDegrees(VE.theta()))
}
...
public boolean inDC(DataBank recPart, int p){
	int status = bank.getShort("status", p);
...
public boolean ele_charge_cut(DataBank recPart, int p){
  if(recPart.getInt("charge",p)==11) return true;
  else return false;
}
