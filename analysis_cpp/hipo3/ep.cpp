...
int main(int argc, char** argv) {
	...
	if(argc==3){
		if(atoi(argv[1])==1){
			cout << "Will assume this hipo file corresponds to: Ebeam = 10.6 GeV, target = H (i.e. RGA)" << endl;
			Ebeam = 10.6; //GeV
			mtar  = mp;
		}
		else if(atoi(argv[1])==2){
			cout << "Will assume this hipo file corresponds to: Ebeam = 10.2 GeV, target = D (i.e. RGB)" << endl;
			Ebeam = 10.2; //GeV
			mtar  = mD;
		}
		inputFile = argv[2];
		...
		cout << "where: A = 1 -> Ebeam = 10.6 GeV, target = H (i.e. RGA)" << endl;
		cout << "         = 2 -> Ebeam = 10.2 GeV, target = D (i.e. RGB)" << endl;
		cout << "=========================" << endl;
		exit(0);
	}
	...
	TH1F * h1_xB    = new TH1F("h1_xB"    ,"h1_xB"    ,20,   0,  2);	PrettyTH1F(h1_xB    ,"x_B"                  ,"Counts",62);
	...
	TH2F * h2_xBQ2=new TH2F("h2_xBQ2", "h2_xBQ2",100,0,2,100,0,12);
	PrettyTH2F(h2_xBQ2, "x_{B}","Q^{2}");
	hipo::reader reader;
